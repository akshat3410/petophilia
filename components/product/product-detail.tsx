    ...
    <>
      ...
      <div className="mx-auto max-w-[1400px] px-8 pt-8">
        <nav className="flex items-center gap-2 text-[12px] text-muted">
          ...
          <Link href={`/category/${product.pet}`} className="capitalize hover:text-accent">
            {product.pet}
          </Link>
          <ChevronRight size={12} />
      ...
      <section className="mx-auto mt-6 grid max-w-[1400px] grid-cols-12 gap-10 px-8 pb-16">
              ...
              {[0, 1, 2, 3].map((i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === 0 ? "border-border" : "border-border",
                  )}
        ...
        <div className="col-span-12 md:col-span-5">
          <div className="rounded-lg bg-white p-8 shadow-soft-md">
            ...
            <h1 className="mt-3 font-serif text-[44px] italic leading-[1] text-primary">
              {product.name}
            </h1>
            <p className="mt-3 text-[14px] text-muted">{product.unit}</p>
            <p className="mt-4 text-[15px] leading-[1.55] text-muted">{product.blurb}</p>

            <div className="mt-5 flex items-center gap-3 font-mono text-[12px] text-muted">
            ...
            <div className="mt-7 space-y-2">
              <button
                ...
                className={cn(
                  ...
                  mode === "once" ? "border-border bg-primary/[0.03]" : "border-border",
                )}
              >
                <div>
                  <p className="font-medium text-primary">One-time purchase</p>
                  <p className="mt-0.5 text-[12px] text-muted">Single delivery.</p>
                </div>
                <p className="font-mono text-[15px] text-primary">{formatPrice(product.price)}</p>
              </button>
              <button
                onClick={() => setMode("sub")}
                className={cn(
                  "flex w-full items-center justify-between rounded-[12px] border p-4 text-left transition-colors",
                  mode === "sub" ? "border-border bg-primary/[0.03]" : "border-border",
                )}
              >
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-primary">Subscribe & save</p>
                    <span className="rounded-full bg-accent/15 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.14em] text-accent">
                      −{Math.round(SUBSCRIBE_DISCOUNT * 100)}%
                    </span>
                  </div>
                  <p className="mt-0.5 text-[12px] text-muted">Every 4 weeks. Skip anytime.</p>
                </div>
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <button
                ...
                className="grid h-12 w-12 place-items-center rounded-full border border-border bg-white text-primary transition-all hover:-translate-y-0.5"
              >
                <Heart size={16} fill={saved ? "currentColor" : "none"} className={saved ? "text-offer" : ""} />
              </button>
            </div>
          ...
          {/* Ingredients card */}
          <div className="mt-6 rounded-2xl border border-accent/15 bg-accent/5 p-8">
            <p className="mono-label">/what's inside</p>
            <h2 className="mt-2 font-serif text-[26px] italic text-primary">Ingredients</h2>
            <ul className="mt-5 space-y-2.5">
              {product.ingredients.map((ing) => (
                <li
                  key={ing}
                  className="flex items-center gap-3 border-b border-border pb-2.5 text-[14px] text-primary"
                >
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {product.tags.map((t) => (
              <span
                key={t}
                className="rounded-full border border-border bg-white px-3.5 py-1.5 font-mono text-[11px] uppercase tracking-[0.12em] text-muted"
              >
                {t}
              </span>
            ))}
      ...
      <section className="mx-auto max-w-[1400px] px-8 pb-24">
        <div className="flex items-end justify-between">
          <div>
            ...
            <h2 className="mt-2 font-serif text-[36px] italic text-primary">Often paired with.</h2>
          </div>
        </div>
